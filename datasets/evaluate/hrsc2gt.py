...
def convert_hrsc_gt(gt_path, dst_path, level=1, eval_difficult= False):
    if os.path.exists(dst_path):
        shutil.rmtree(dst_path)
    os.mkdir(dst_path)
    with open(gt_path,'r') as f:
        files = [x.strip('\n').replace('.jpg','.xml').replace('AllImages','Annotations') for x in f.readlines()]
...
if __name__ == "__main__":
    level = 1
    gt_path = '/data-tmp/das3/HRSC2016/test.txt' 
    dst_path = '/data-tmp/das3/datasets/evaluate/ground-truth'
    eval_difficult = False
    convert_hrsc_gt(gt_path, dst_path, level, eval_difficult)
